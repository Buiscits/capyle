...
def transition_func(grid, neighbourstates, neighbourcounts, decaygrid, initial_terrain, topology_grid):
    ...
    def probability_p_burn(x, y):
        ...
        wind_probabilities = probability_p_w(
            [0, -1], 0.1, fire_direction(x, y))
    ...
    def calculate_slope_fire_spread_probability(row, column):
        ...
        neighbour_terrain_fire_spread_probabilities = calculate_terrain_spread_probabilities(
            row, column)
        #print("MAX: ", max(neighbour_terrain_fire_spread_probabilities))
        # See if slope small enough to make block catch fire
        cells_that_should_spread_fire = np.array(
            neighbour_terrain_fire_spread_probabilities) < 0.8
    ...
    def calculate_terrain_spread_probabilities(row, column):
        ...
        down_slope = calculate_slope(
            terrain, topology_grid[row][col - 1], False)
        right_slope = calculate_slope(
            terrain, topology_grid[row + 1][col], False)
        left_slope = calculate_slope(
            terrain, topology_grid[row - 1][col], False)

        # Diagonal
        top_left_slope = calculate_slope(
            terrain, topology_grid[row - 1][col - 1], True)
        bottom_left_slope = calculate_slope(
            terrain, topology_grid[row + 1][col - 1], True)
        top_right_slope = calculate_slope(
            terrain, topology_grid[row - 1][col + 1], True)
        bottom_right_slope = calculate_slope(
            terrain, topology_grid[row + 1][col + 1], True)

        slopes = [up_slope, down_slope, left_slope, right_slope, top_left_slope,
                  top_right_slope, bottom_left_slope, bottom_right_slope]

        fire_probabilities = list(
            map(lambda x: terrain_fire_rates[x[0]] * math.exp(1 * x[1]), slopes))
...
def setup(args):
    ...
    config.state_colors = np.array(
        [chaparral_color, lake_color, dense_forest_color, scrubland_color, town_color, fire_color])/255
